...
export class DbScannerComponent {
  ...
  get safeMetadataUrl() {
    return this.sanitizer.bypassSecurityTrustUrl(this.sharepointMetadataUrl);
  }

  get safeMigrationUrl() {
    return this.sanitizer.bypassSecurityTrustUrl(this.sharepointMigrationUrl);
  }
  ...
  metadataFile =
    '/output/Metadata_Report.docx';

  migrationFile =
    '/output/Migration_Assessment.docx';
  ...
  sourceChanged() {
    ...
    // Pre-fill from the last-saved connection for this source, if any -
    // still fully editable, just saves retyping. Silently keeps the blank
    // form on error/no-saved-connection.
